...
*
* File Name: Focaltech_test_ft5682.c
*
    ...
    struct fts_test *tdata, int *adc, u8 *ab_ch, bool *result)
{
    ...
    for (i = 0; i < ch_num; i++) {
        code = adc[i];
        denominator = code1 - code;
        if (denominator == 0) {
            short_res[i] = min_cc;
        } else {
            numerator = (code - offset + 480) * 113;
...
static int ft5682_rawdata_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    int *rawdata = NULL;
...
static int ft5682_uniformity_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int index = 0;
    int row = 0;
...
static int ft5682_scap_cb_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    u8 wc_sel = 0;
    u8 sc_mode = 0;
    ...
    if (tdata->sc_node.node_num * 3 > tdata->buffer_length) {
        FTS_TEST_SAVE_ERR("scap cb num(%d) > buffer length(%d)",
                          tdata->sc_node.node_num * 3,
                          tdata->buffer_length);
        ret = -EINVAL;
        goto test_err;
    }
    ...
    /*high mode*/
    fw_wp_check = !hc_sel;
    if (thr->basic.scap_cb_hi_check && hc_sel) {
        scb_tmp = scap_cb + scb_cnt;
        /* 1:waterproof 0:non-waterproof */
        ...
        /* compare */
        tx_check = ((hc_sel == 1) || (hc_sel == 3));
        rx_check = ((hc_sel == 2) || (hc_sel == 3));
...
static int ft5682_scap_rawdata_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    bool tmp2_result = false;
    ...
    if (thr->basic.scap_rawdata_hi_check && hc_sel) {
        srawdata_tmp = scap_rawdata + srawdata_cnt;
...
static int ft5682_short_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int ch_num = 0;
...
test_err:
    ret = fts_test_write_reg(FACTROY_REG_SHORT2_TEST_EN, 0x06);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("restore short state fails");
    }
    sys_delay(200);
...
static int ft5682_panel_differ_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int i = 0;
...
static int start_test_ft5682(void)
{
    int ret = 0;
    ...
    if (true == test_item->rawdata_test) {
        ret = ft5682_rawdata_test(tdata, &temp_result);
    ...
    if (true == test_item->rawdata_uniformity_test) {
        ret = ft5682_uniformity_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->scap_cb_test) {
        ret = ft5682_scap_cb_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->scap_rawdata_test) {
        ret = ft5682_scap_rawdata_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->short_test) {
        ret = ft5682_short_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->panel_differ_test) {
        ret = ft5682_panel_differ_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
...
struct test_funcs test_func_ft5682 = {
    .ctype = {0x87},
    .hwtype = IC_HW_MC_SC,
    .key_num_total = 0,
    .mc_sc_short_v2 = true,
