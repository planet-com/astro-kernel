	...
	&& hx_touch_data->hx_rawdata_buf[3] == diag_cmd) {
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) { /*mutual*/
				mutual_data[index + i] =
					((int8_t)hx_touch_data->hx_rawdata_buf[
					 i * 2 + 4 + 1]) * 256 +
...
static int hx_read_guest_info(void)
{
	...
	custom_info_temp++) {
		...
		if (checksum != 0) {
			...
			memcpy(&g_guest_info_data->g_guest_str_in_format[
					custom_info_temp][0],
				g_checksum_str, (int)strlen(g_checksum_str));
			memcpy(&g_guest_info_data->g_guest_str[
					custom_info_temp][0],
				g_checksum_str, (int)strlen(g_checksum_str));
			continue;
		}
...
/* uint8_t j = 0; */
	...
	if (G_POWERONOF == 1) {
		...
		I("%s: write reply retry = %d, data = 0x%02X%02X%02X%02X\n",
				__func__, retry, recv_data[0],
