...
#define FW_VERSION_INFO_ADDR_BRB	0x10014

#define GOODIX_IC_INFO_MAX_LEN		1024
#define GOODIX_IC_INFO_ADDR_BRA		0x10068
#define GOODIX_IC_INFO_ADDR_BRB		0x10070
...
static int brl_select_spi_mode(struct goodix_ts_core *cd)
{
	...
	if (cd->bus->bus_type == GOODIX_BUS_TYPE_I2C ||
			cd->bus->ic_type == IC_TYPE_BERLIN_B)
		return 0;
...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (!cd->avdd || !cd->iovdd) {
		...
		if (on) {
			if (cd->board_data.avdd_gpio > 0 && cd->board_data.iovdd_gpio > 0) {
			...
			if (ret < 0) {
				...
				if (cd->board_data.avdd_gpio > 0 && cd->board_data.iovdd_gpio > 0) {
					gpio_direction_output(cd->board_data.iovdd_gpio, 0);
					gpio_direction_output(cd->board_data.avdd_gpio, 0);
				}
				return ret;
			}
			...
			struct goodix_fw_version *version)
{
	...
	if (cd->bus->ic_type == IC_TYPE_BERLIN_A)
		...
		fw_addr = FW_VERSION_INFO_ADDR_BRB;
	...
	struct goodix_ic_info *ic_info, bool isPrint)
{
	...
	if (cd->bus->ic_type == IC_TYPE_BERLIN_A)
		...
		ic_addr = GOODIX_IC_INFO_ADDR_BRB;
