...
static int gesture_module_init(void *data)
{
	int ret;
	int i;
	int retry = 10;
	...
	if (retry < 0) {
		ts_err("core module is not exist, gesture module load failed!");
		return -EINVAL;
	}
...
static int __init goodix_gsx_gesture_init(void)
{
	...
	init_thrd = kthread_run(gesture_module_init,
				NULL, "gesture_module_init");
	if (IS_ERR_OR_NULL(init_thrd)) {
		ts_err("Failed to create gesture init thread:%ld",
		       PTR_ERR(init_thrd));
		return -EFAULT;
	}
