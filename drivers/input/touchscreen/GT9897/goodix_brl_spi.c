 ...
 */
int goodix_spi_read_bra(struct device *dev, unsigned int addr,
...
int goodix_spi_read_brb(struct device *dev, unsigned int addr,
	unsigned char *data, unsigned int len)
{
	struct spi_device *spi = to_spi_device(dev);
	u8 *rx_buf = NULL;
...
static int goodix_spi_probe(struct spi_device *spi)
{
	...
	if (goodix_spi_bus.ic_type == IC_TYPE_BERLIN_A)
		...
		goodix_spi_bus.read = goodix_spi_read_brb;
