...
static int inspect_module_init(void *data)
{
	int ret;
	int retry = 10;
	...
	if (retry < 0) {
		ts_err("core module is not exist, inspect module load failed!");
		return -EINVAL;
	}
	...
	ts_info("inspect module load success");
	return 0;

err_out:
	ts_err("inspect module load failed!");
	return ret;	
}

static int __init goodix_inspect_init(void)
{
	struct task_struct *init_thrd;

	ts_info("inspect module init");
	module_initialized = false;
	init_thrd = kthread_run(inspect_module_init,
				NULL, "inspect_module_init");
	if (IS_ERR_OR_NULL(init_thrd)) {
		ts_err("Failed to create inspect init thread:%ld",
		       PTR_ERR(init_thrd));
		return -EFAULT;
	}
...
static void __exit goodix_inspect_exit(void)
{
	struct kobject *def_kobj = goodix_get_default_kobj();
