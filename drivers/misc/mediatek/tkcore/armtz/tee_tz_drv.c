...
static bool verify_fill_clnt_login(struct teesmc_meta_open_session *meta,
					uint32_t method, uint32_t gid_in)
{
	int i;
	uint32_t appid, userid;
	...
	meta->clnt_login = method;
	memset(meta->clnt_uuid, 0, sizeof(meta->uuid));

	switch (meta->clnt_login) {
		case TEEC_LOGIN_PUBLIC:
			break;

		case TEEC_LOGIN_USER:
			memcpy(meta->clnt_uuid, &userid, sizeof(userid));
			break;

		case TEEC_LOGIN_APPLICATION:
			memcpy(meta->clnt_uuid + sizeof(uint32_t),
				&appid, sizeof(appid));
			break;

		case TEEC_LOGIN_USER_APPLICATION:
			//id of user & app are all included in uid
			memcpy(meta->clnt_uuid, &userid, sizeof(userid));
			memcpy(meta->clnt_uuid + sizeof(uint32_t),
				&appid, sizeof(appid));
			break;
		...
		case TEEC_LOGIN_GROUP:
			...
			if (found_group == false && gid_in != uid) {
				return false;
			}

			memcpy(meta->clnt_uuid + sizeof(uint32_t) + sizeof(uint32_t),
				&gid_in, sizeof(gid_in));
			break;
	}
...
static int tz_open(struct tee_session *sess, struct tee_cmd *cmd)
{
	...
	if (!verify_fill_clnt_login(meta,
				cmd->connection_method, cmd->connection_data)) {
		ret = -EPERM;
		goto out;
	}
