			 ...
			 struct tee_cmd *cmd)
{
	...
	if (cmd_io->data == NULL) {
		cmd->connection_method = TEEC_LOGIN_PUBLIC;
		cmd->connection_data = 0;
	} else if (cmd_io->data_size >= sizeof(uint32_t)) {
		if (tee_context_copy_from_client(ctx,
				&cmd->connection_method,
				cmd_io->data, sizeof(uint32_t))) {
			return -EFAULT;
		}

		switch (cmd->connection_method) {
			...
			case TEEC_LOGIN_GROUP_APPLICATION:
				if (cmd_io->data_size !=
					...
					pr_warn("tkcoredrv: invalid login group size\n");
					return -EINVAL;
				}

				if (tee_context_copy_from_client(ctx,
						(void *) &cmd->connection_data,
						(void *) (((unsigned char *) cmd_io->data) + sizeof(uint32_t)),
						sizeof(uint32_t))) {
					return -EFAULT;
				}
				break;
		...
		pr_warn("tkcoredrv: invalid connection data size\n");
		return -EINVAL;
	}
