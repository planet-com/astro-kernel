					...
					enum TYPEC_CONNECTION_STATE state)
{
	...
	if (strcmp((typec_state_name[state]), "Attached.SNK") == 0){
		printk("aeon usbr is charger plugin\n");
	} else if (strcmp((typec_state_name[state]), "Unattached.SNK") == 0){
		printk("aeon usbr is charger plugout\n");
	}
	usbr_cc1 = typec_get_cc1();
	usbr_cc2 = typec_get_cc2();
