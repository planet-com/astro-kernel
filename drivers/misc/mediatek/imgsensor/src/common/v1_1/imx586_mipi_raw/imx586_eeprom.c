...
#define MTK_IDENTITY_VALUE_ZS 0x46
#define LRC_SIZE 384
#define DCC_SIZE 96

#define QSC_SIZE 2304
BYTE IMX586_QSC_data[QSC_SIZE]= {0};
...
static struct EEPROM_PDAF_INFO *get_eeprom_pdaf_info(void)
{
	...
	if (pinfo == NULL) {
		read_imx586_eeprom(0x1, read_data, 1);
		//if (((read_data[3] << 24) |
		//     (read_data[2] << 16) |
		//     (read_data[1] << 8) |
		//     read_data[0]) == MTK_IDENTITY_VALUE) {
		if (read_data[0] == MTK_IDENTITY_VALUE_ZS) {
...
#if 1
unsigned int read_imx586_QSC(void)
{
	static unsigned int readed_size;
	struct EEPROM_PDAF_INFO *pinfo = get_eeprom_pdaf_info();

	printk("read imx586 QSC, addr = %d, size = %u\n",
		pinfo->QSC_addr, pinfo->QSC_size);

	mutex_lock(&gimx586_eeprom_mutex);
	if ((readed_size == 0) &&
	    read_imx586_eeprom(pinfo->QSC_addr,
			       IMX586_QSC_data, pinfo->QSC_size)) {
		readed_size = pinfo->QSC_size;
	}
	mutex_unlock(&gimx586_eeprom_mutex);
