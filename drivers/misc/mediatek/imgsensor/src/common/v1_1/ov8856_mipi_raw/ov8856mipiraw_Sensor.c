...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	/* Preview */
	{ 3296, 2480, 0, 12, 3296, 2456, 1648, 1228,
	8,	2, 1632, 1224, 0, 0, 1632, 1224},
	/* capture */
	{ 3296, 2480, 0, 12, 3296, 2456, 3296, 2456,
	16, 4, 3264, 2448, 0, 0, 3264, 2448},
	/* video*/
	{ 3296, 2480, 0, 12, 3296, 2456, 3296, 2456,
	16, 4, 3264, 2448, 0, 0, 3264, 2448},
	/*hight speed video */
...
#if MULTI_WRITE
		/* Write when remain buffer size is */
		/*	less than 3 bytes or reach end of data */
...
static void set_shutter(kal_uint16 shutter)
{
	...
	/* 0x3500, 0x3501, 0x3502 will increase*/
	/*VBLANK to get exposure larger than frame exposure */
...
static void preview_setting(void)
{
	...
	ov8856_table_write_cmos_sensor(addr_data_pair_preview_ov8856,
			sizeof(addr_data_pair_preview_ov8856) /
			sizeof(kal_uint16));
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) |
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("i2c w_id:0x%x,senid:0x%x,ver=0x%x\n",
...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	if (addr != 0) {
		...
		r_awb_data =
		(read_cmos_sensor(addr + 2) << 2) + ((temp >> 6) & 0x03);
		gr_awb_data =
		(read_cmos_sensor(addr + 3) << 2) + ((temp >> 4) & 0x03);
		gb_awb_data =
		(read_cmos_sensor(addr + 4) << 2) + ((temp >> 2) & 0x03);
		b_awb_data = (read_cmos_sensor(addr + 5) << 2) + (temp & 0x03);
		LOG_INF("r = %d,gr = %d,b = %d,gb_awb_data = %d\n",
			r_awb_data, gr_awb_data, b_awb_data, gb_awb_data);
		(*otp_ptr).rg_ratio =
			((r_awb_data * 1023/
			((gr_awb_data + gb_awb_data)/2)) * 10 + 5)/10;
		...
		if (addr == 0x7011)
			checksum2 = checksum2 +
			read_cmos_sensor(0x720B) + read_cmos_sensor(0x720C);
		else
			checksum2 = checksum2 +
			read_cmos_sensor(0x720D) + read_cmos_sensor(0x720E);
...
int apply_otp(struct otp_struct *otp_ptr)
{
	int RG_Ratio_Typical =
		(*otp_ptr).rg_Golden, BG_Ratio_Typical = (*otp_ptr).bg_Golden;
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			LOG_INF("Warning:cfps %d isn't spt,so use cap1's:%d\n",
