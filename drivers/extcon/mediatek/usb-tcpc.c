		...
		unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			tcpc_otg_enable();
			printk("aeon usbr is otg plugin\n");
			if (aeon_usbl_state == 0 || aeon_usbl_state == 3){
				
			} else if (aeon_usbl_state == 1){
			...
			if (aeon_usbl_state == 2 && aeon_usbr_state == 1) {
				aeon_usbr_state = 0;
			} else {
				aeon_usbr_state = 0;
				if (aeon_usbl_state == 0){
					usb_gpio_init();
				} else if (aeon_usbl_state == 1){
				...
				if (aeon_usbl_state == 2 || aeon_usbr_state == 2){
					printk("usbl otg, aeon usbr is charger plugout\n");
				} else {
					if (otg_on) {
						pr_info("%s OTG Plug out\n", __func__);
						tcpc_otg_disable();
						printk("aeon usbr is otg plugout\n");
					} else {
