...
static void print_send_buffer(struct nfc_dev *nfc_dev, unsigned char *buf, int len)
{
	...
	pr_debug(&nfc_dev->client->dev, "%3d > %s\n", len, output);
}

static void print_recv_buffer(struct nfc_dev *nfc_dev, unsigned char *buf, int len)
{
	unsigned char output[MAX_I2C_DUMP_SIZE * 2 + 1];
	int i;
...
#if DUMP_TRANSCIEVING_BUF
	print_recv_buffer(nfc_dev, tmp, ret);
#endif	
...
#if DUMP_TRANSCIEVING_BUF
	print_send_buffer(nfc_dev, tmp, count);
#endif	
    /* delay of 1ms for slow devices*/
    udelay(1000);
    kfree(tmp);
