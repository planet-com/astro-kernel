...
//ycj
#if 0
void gpio2_pull(void)
{
	struct mtk_pinctrl *hw = dev_get_drvdata(dev_pub);
	const struct mtk_pin_desc *desc;

	desc = (const struct mtk_pin_desc *)&hw->soc->pins[2];
	mtk_pinconf_bias_set_combo(hw, desc, 1, 1);

}
EXPORT_SYMBOL(gpio2_pull);
#endif
//
