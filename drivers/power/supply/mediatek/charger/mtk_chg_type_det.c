	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	if (!cti->ignore_usb) {
		...
		if ((mtk_chg->chg_type == STANDARD_HOST) ||
			...
			if (aeon_usbl_state == 2 || aeon_usbr_state == 2) {
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			//aeon add
			printk("%s:usbr_cc1=%d, usbr_cc2=%d\n", __func__, usbr_cc1, usbr_cc2);
			if ((usbr_cc1 == 0 && usbr_cc2 == 5)||(usbr_cc1 == 5 && usbr_cc2 == 0)){
				if (aeon_usbl_state == 1){
					aeon_gpio_set("usbl_chg_en_high"); //GPIO34
				} else if (aeon_usbl_state == 2){
					mt6360_enable_uug_on(0);
				} else if (aeon_usbl_state == 3 || aeon_usbl_state == 0){
			...
			if (aeon_usbr_state == 1) {
				...
				if (aeon_usbl_state == 1){
					...
					printk("usbl otg, usbr charging plugout\n");
					mt6360_enable_uug_on(1);
				} else if (aeon_usbl_state == 3){
				
				} else if (aeon_usbl_state == 0){
					usb_gpio_init();
				}
			}
			//end
...
bool mt_charger_plugin(void)
{
	...
	if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || boot_mode == LOW_POWER_OFF_CHARGING_BOOT) {
		if (aeon_gpio_get("hdmi_hpd_state") == 0 && aeon_gpio_get("usbl_charger_state") == 0) {
			return true;
		}
	}
	return cti->plugin;
